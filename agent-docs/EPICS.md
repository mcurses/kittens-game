...
| 37 | **Building Control Granularity Parity** — restore legacy distinction between per-building count controls (`togglable`) and binary full-on/full-off controls (`togglableOnOff`), with engine-owned control metadata, batch action surface, and smelter-style web controls | 34, 35 | P1 | ⏳ Not Started |
