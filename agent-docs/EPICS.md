...
| 37 | **Building Control Granularity Parity** — restore legacy distinction between per-building count controls (`togglable`) and binary full-on/full-off controls (`togglableOnOff`), with engine-owned control metadata, batch action surface, and smelter-style web controls | 34, 35 | P1 | 🚧 In Progress |
| 38 | **Trade Shortcut Quantity Parity** — replace fixed caravan shortcuts with legacy dynamic `tradeHalf` / `tradeFifth` behavior and matching amount calculations | 35 | P2 | ⏳ Not Started |
| 39 | **Workshop Engineer Assignment Engine** — add per-craft engineer assignment state and action surface as the engine prerequisite for deferred mechanization UI parity | 09, 35 | P2 | ⏳ Not Started |
