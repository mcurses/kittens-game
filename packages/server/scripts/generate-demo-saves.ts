...
function setBuildings(state: GameState, entries: Record<string, number>): GameState {
  return mutate(state, (draft) => {
...
function setResources(state: GameState, entries: Record<string, number>): GameState {
  return mutate(state, (draft) => {
...
function generateEndgame(): SerializedGameState {
  ...
  state = setResources(state, {
    ...
    steel: 5_000_000_000,
    gear: 1_000_000_000,
    concrate: 1_000_000_000,
    alloy: 100_000_000,
    eludium: 1_000_000,
    scaffold: 100_000_000_000,
    ship: 500_000,
    parchment: 50_000_000,
    manuscript: 20_000_000,
    compedium: 10_000_000,
    blueprint: 50_000_000,
    kerosene: 500_000_000,
    starchart: 1_000_000_000,
    gflops: 500_000,
    temporalFlux: 5_000,
  });
  state = setBuildings(state, {
  ...
  state = mutate(state, (draft) => {
    ...
    if (draft.space) {
      ...
      for (const [name, val] of Object.entries({
        ...
        orbitalArray: 50,
        sunlifter: 40,
        containmentChamber: 10,
        heatsink: 15,
        cryostation: 50,
        spaceBeacon: 20,
        terraformingStation: 15,
        hydroponics: 80,
        tectonic: 5,
      })) {
...
function emit(name: string, save: SerializedGameState): void {
  ...
  const _sizeKb = (JSON.stringify(save).length / 1024).toFixed(1);
}
