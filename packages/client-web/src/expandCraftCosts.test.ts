...
describe("expandCraftCosts", () => {
  it("returns direct prices at depth 1 even when already met", () => {
    ...
    expect(result.get("wood")).toMatchObject<IngredientNode>({ amount: 100, depth: 1, parentName: "" });
    expect(result.get("minerals")).toMatchObject<IngredientNode>({ amount: 200, depth: 1, parentName: "" });
  });
  ...
  it("expands one level of ingredients when deficit exists at depth 1", () => {
    ...
    expect(result.get("manuscript")).toMatchObject({ depth: 2, parentName: "compendium", amount: 250 });
    expect(result.get("science")).toMatchObject({ depth: 2, parentName: "compendium", amount: 50_000 });
  });
  ...
  it("recurses to depth 3 for a full chain: compendium → manuscript → parchment", () => {
    ...
    expect(result.get("manuscript")).toMatchObject({ depth: 2, parentName: "compendium", amount: 50 });
    ...
    expect(result.get("parchment")).toMatchObject({ depth: 3, parentName: "manuscript", amount: 1250 });
